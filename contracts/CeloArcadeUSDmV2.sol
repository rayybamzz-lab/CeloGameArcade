...
contract CeloArcadeUSDmV2 {
    ...
    uint256 public constant ENTRY_FEE = 0.1 ether; // 0.1 USDm (18 decimals)
    ...
    constructor(address usdmTokenAddress) {
    ...
    function depositToPlay() external nonReentrant {
        _safeTransferFrom(address(usdmToken), msg.sender, address(this), ENTRY_FEE);

        uint256 creatorShare = (ENTRY_FEE * CREATOR_BPS) / BPS_DENOMINATOR;
        uint256 prizeShare = ENTRY_FEE - creatorShare;
        ...
        emit AccessGranted(msg.sender, ENTRY_FEE, prizeShare, seasonNumber);
    }
