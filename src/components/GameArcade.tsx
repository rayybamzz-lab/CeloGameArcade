...
export default function GameArcade() {
  ...
  const SnakeGame = ({ onEnd }: { onEnd: (score: number) => void }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '12px' }}>
        ...
        {state === 'play' && (<div style={{ marginTop: '12px' }}><div style={{ display: 'flex', justifyContent: 'center', marginBottom: '6px' }}><button type="button" onClick={() => chDir({ x: 0, y: -1 })} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#f66,#e55)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>↑</button></div><div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}><button type="button" onClick={() => chDir({ x: -1, y: 0 })} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#f66,#e55)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>←</button><button type="button" onClick={() => chDir({ x: 0, y: 1 })} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#f66,#e55)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>↓</button><button type="button" onClick={() => chDir({ x: 1, y: 0 })} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#f66,#e55)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>→</button></div></div>)}
      </div>
    );
  };
  ...
  const SpaceGame = ({ onEnd }: { onEnd: (score: number) => void }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '12px' }}>
        ...
        {state === 'play' && (<div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}><button type="button" onMouseDown={() => setSx((x) => Math.max(10, x - 8))} onTouchStart={() => setSx((x) => Math.max(10, x - 8))} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#93f,#72c)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>←</button><button type="button" onClick={shoot} onTouchStart={shoot} style={{ width: '70px', height: '50px', background: 'linear-gradient(135deg,#f44,#c22)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer' }}>🔥 FIRE</button><button type="button" onMouseDown={() => setSx((x) => Math.min(90, x + 8))} onTouchStart={() => setSx((x) => Math.min(90, x + 8))} style={{ width: '50px', height: '50px', background: 'linear-gradient(135deg,#93f,#72c)', borderRadius: '10px', border: 'none', color: '#fff', fontSize: '20px', cursor: 'pointer' }}>→</button></div>)}
      </div>
    );
  };
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', padding: '14px', fontFamily: 'system-ui' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
        ...
        {isConnected ? (
          ...
          <button type="button" onClick={handleConnectClick} style={{ background: 'linear-gradient(135deg,#0f8,#0a6)', border: 'none', borderRadius: '16px', padding: '10px 16px', color: '#fff', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>Connect Wallet</button>
        )}
      </div>
  ...
  // DIFFICULTY MODAL
  const Modal = () => (
    <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '14px' }}>
      <div style={{ background: 'linear-gradient(135deg,#1a1a3e,#0f0c29)', borderRadius: '20px', padding: '20px', maxWidth: '320px', width: '100%', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}><button type="button" onClick={() => setShowSelect(false)} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '12px', color: '#fff', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Cancel</button><button type="button" onClick={() => { setShowSelect(false); setView('game'); }} style={{ flex: 2, padding: '12px', background: `linear-gradient(135deg,${game?.color},${game?.color}cc)`, border: 'none', borderRadius: '12px', color: '#000', fontSize: '14px', fontWeight: '800', cursor: 'pointer', boxShadow: `0 6px 20px ${game?.color}44` }}>🎮 START</button></div>
      </div>
    </div>
  );
  ...
  // WALLET MODAL
  const WalletModal = () => (
    <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '14px' }}>
